...
def select_elements_starting_with_vowel(array)
	array.select {|element| element.chars.first =~ /\A[aeiou]/}
end
...
def number_of_elements_that_are_palindromes(array)
	array.select{|n| n==n.reverse}.count
end
...
def titleize_a_string(string)
	...
	s = string.split(' ').each{|n| n.capitalize! unless n=='the' || n=='and'}.join(' ')
	a = s.split(' ')
	a[0].capitalize!
	a.join(' ')
end
...
def check_a_string_for_special_characters(string)
	string.chars.any? {|val| /[a-zA-Z0-9]/ !~ val}
end
...
def word_count_a_file(file_path)
	results = []
	File.open(file_path, "r").each { |line| results << line }
...
def is_a_2014_bank_holiday?(date)
	bank_holidays_2014 = [Time.new(2014, 1, 1), Time.new(2014, 4, 18), Time.new(2014, 4, 18), Time.new(2014, 4, 21), Time.new(2014, 5, 26), Time.new(2014, 8, 25), Time.new(2014, 12, 25), Time.new(2014, 12, 26)]
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
	one_year = 3.156e+7
	birthday = birthday + one_year until birthday.friday?
